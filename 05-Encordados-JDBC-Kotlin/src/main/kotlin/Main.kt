...
import repository.raqueta.RaquetasRepositoryImpl
import repository.tenista.TenistasRepositoryImpl
...
fun main() {
    ...
    val controller = MutuaController(RaquetasRepositoryImpl(), TenistasRepositoryImpl(RaquetasRepositoryImpl()))
    ...
    controller.deleteRaqueta(raquetas[0])

    // Obtenemos todas las raquetas ordenadas por marca
    raquetas = controller.getRaquetas().sortedBy { it.marca }
    raquetas.forEach { println(it) }
